...
const propertySchema = new mongoose.Schema({
  ...
  maximumGuest: {
    type: Number,
    required: [true, "Please provide the maximum number of guests that can be accommodated"]
  },
  amenities: [
    {
      name: {
...
// Middleware to generate slug from property name before saving
propertySchema.pre("save", function (next) {
  this.slug = slugify(this.propertyName, { lower: true });
  next();
});

// Middleware to normalize city name before saving
propertySchema.pre("save", function (next) {
